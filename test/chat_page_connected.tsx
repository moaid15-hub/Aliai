...
export default function ChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'مرحباً! أنا Oqool AI، مساعدك الذكي. كيف يمكنني مساعدتك اليوم؟ 🚀',
      timestamp: new Date()
    }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch('https://api.oqool.net/chat', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          conversation_id: null, // Create new conversation or use existing
