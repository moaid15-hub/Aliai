...
export const intelligentCategories: Record<string, CategoryConfig> = {
  // محادثة عامة - OpenAI أولاً، Claude احتياطي
  simpleChat: {
  ...
  // برمجة - Claude أولاً، DeepSeek للفشل
  simpleCoding: {
  ...
  // برمجة عميقة ومعمارية - Claude أولاً، DeepSeek للفشل
  deepCoding: {
  ...
  // أسئلة معقدة وتحليلية - OpenAI أولاً، Claude ثم DeepSeek
  complexQuestions: {
  ...
  // ترجمة - OpenAI أولاً، Claude ثم DeepSeek
  translation: {
  ...
  // رياضيات - Claude أولاً، DeepSeek للفشل
  mathematics: {
  ...
  // كتابة إبداعية - OpenAI أولاً، Claude ثم DeepSeek
  creativeWriting: {
...
// 🔄 مزود بديل - OpenAI → Claude → DeepSeek
export function getAlternativeProvider(provider: string): string {
  const alternatives: Record<string, string> = {
    'openai': 'claude',      // إذا فشل OpenAI → Claude
    'claude': 'deepseek',    // إذا فشل Claude → DeepSeek
    'deepseek': 'openai',    // إذا فشل DeepSeek → OpenAI (حلقة)
...
// 📊 تدريج النموذج حسب طول المحادثة
...
// 🎯 جدول التدريج للنماذج
export const modelScaling: Record<string, ModelScale[]> = {
  openai: [
    { messageCount: 0, model: 'gpt-4o-mini', description: 'للمحادثات القصيرة والبسيطة' },
    { messageCount: 10, model: 'gpt-4o', description: 'للمحادثات المتوسطة' },
    { messageCount: 20, model: 'gpt-4-turbo', description: 'للمحادثات الطويلة والمعقدة' }
  ],
  claude: [
    { messageCount: 0, model: 'claude-3-haiku', description: 'للمحادثات القصيرة' },
    { messageCount: 10, model: 'claude-3-sonnet', description: 'للمحادثات المتوسطة' },
    { messageCount: 20, model: 'claude-3-opus', description: 'للمحادثات الطويلة' }
  ],
  deepseek: [
    { messageCount: 0, model: 'deepseek-chat', description: 'نموذج واحد لكل الأطوال' }
  ]
};

// 🎯 اختيار النموذج المناسب حسب عدد الرسائل
export function selectModelByMessageCount(provider: string, messageCount: number): string {
  const scales = modelScaling[provider];

  if (!scales || scales.length === 0) {
    return 'gpt-4o-mini'; // افتراضي
  }

  // إيجاد النموذج المناسب
  let selectedModel = scales[0].model;

  for (const scale of scales) {
    if (messageCount >= scale.messageCount) {
      selectedModel = scale.model;
    } else {
      break;
    }
  }

  console.log(`📊 عدد الرسائل: ${messageCount} → النموذج: ${selectedModel}`);
