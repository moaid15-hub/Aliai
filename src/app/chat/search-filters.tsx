...
interface SearchFiltersProps {
  ...
  activeFilter: 'all' | 'google' | 'youtube' | 'wikipedia' | 'stackoverflow' | 'github';
  onFilterChange: (filter: 'all' | 'google' | 'youtube' | 'wikipedia' | 'stackoverflow' | 'github') => void;
}
...
export const SearchFilters: React.FC<SearchFiltersProps> = ({ sources, activeFilter, onFilterChange }) => {
  ...
  const filters = [
    {
      ...
      count: (sources.google?.length || 0) + (sources.youtube?.length || 0) + (sources.wikipedia?.length || 0) + (sources.stackoverflow?.length || 0) + (sources.github?.length || 0),
  ...
  // فلترة الفلاتر التي لها نتائج فقط (أو الكل دائماً)
  const visibleFilters = filters.filter(f => f.key === 'all' || f.count > 0);

  if (visibleFilters.length <= 1) return null; // لا نعرض الفلاتر إذا كان هناك فلتر واحد فقط
