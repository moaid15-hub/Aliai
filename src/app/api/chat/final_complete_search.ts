...
function detectBestSource(query: string): string[] {
  ...
  return sources.length === 0 ? ['google'] : sources.slice(0, 3).map(s => s.name);
}
...
async function smartMultiSourceSearch(query: string, maxResults: number = 5): Promise<MultiSourceResponse> {
  ...
  console.log('🎯 المصادر:', detectedSources);
  ...
  let primarySource = validResults[0];
  const additionalSources = validResults.slice(1);
